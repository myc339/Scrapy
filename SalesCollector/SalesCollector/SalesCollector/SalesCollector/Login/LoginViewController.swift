...
//        print(tableView.contentOffset.y)
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + seconds) {
            ...
            guard let user = try! self.managedObjectContext?.fetch(UserInfo.fetchRequest()) as? [UserInfo] else{return }
                    print("load cached")
                    guard let email = user.first?.emailaddress,let pwd = user.first?.password else{
                        self.authKey = false
                        return}
                    print(email)
            let listQuery = ListUsersQuery(filter: TableUsersFilterInput(emailAddress:TableStringFilterInput(eq:email),password: TableStringFilterInput(eq:pwd)), limit: self.fixedLimit
                    )
            self.appSyncClient?.fetch(query: listQuery, cachePolicy: .fetchIgnoringCacheData) {(result, error) in
                        if error != nil {
